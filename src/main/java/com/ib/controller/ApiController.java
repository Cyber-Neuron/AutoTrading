...
public class ApiController implements EWrapper {
	...
	private int m_reqId; // used for all requests except orders; designed not to
							// conflict with m_orderId
	...
	private HashMap<Integer, StrategyMktDataRow> m_strategyMktDataMap = new HashMap<Integer, StrategyMktDataRow>();
	private HashMap<Integer, StrategyHisDataRow> m_hisDataMapStrategy = new HashMap<Integer, StrategyHisDataRow>();
	private boolean m_connected = false;
	private boolean isRunStrategy = false;
	...
	public void setRunStrategy(boolean isRunStrategy) {
		this.isRunStrategy = isRunStrategy;
	}

	public boolean getRunStrategy() {
		return this.isRunStrategy;
	}
	...
	public void nextValidId(int orderId) {
		m_orderId = orderId;
		m_reqId = m_orderId + 10000000; // let order id's not collide with other
										// request id's
		m_connected = true;
	...
	public void reqTopMktDataStrategy(Contract contract, String genericTickList, boolean snapshot,
			StrategyMktDataRow sendReqRecord) {
		if (!checkConnection())
			return;
		int reqId = m_reqId++;
		m_strategyMktDataMap.put(reqId, sendReqRecord);
		m_client.reqMktData(reqId, contract, genericTickList, snapshot, Collections.<TagValue>emptyList());
		sendEOM();
	}
	...
	public void tickPriceStrategy(int reqId, int tickType, double price, int canAutoExecute) {
		StrategyMktDataRow stockInfo = m_strategyMktDataMap.get(reqId);
		if (stockInfo != null) {
			switch (TickType.get(tickType)) {
			case LAST:
				stockInfo.setLastPrice(price);
				break;
			case BID:
				stockInfo.setBidPrice(price);
				break;
			case ASK:
				stockInfo.setAskPrice(price);
				break;
			default:
				break;
			}
	...
	public interface IOrderHandler {
		...
		void orderStatus(OrderStatus status, double filled, double remaining, double avgFillPrice, long permId,
	...
	public void reqHistoricalData(Contract contract, String endDateTime, int duration, DurationUnit durationUnit,
		...
		m_client.reqHistoricalData(reqId, contract, endDateTime, durationStr, barSize.toString(), whatToShow.toString(),
				rthOnly ? 1 : 0, 2, Collections.<TagValue>emptyList());
		sendEOM();
	}
	ArrayList<Bar> historyData = new ArrayList<Bar>();
    private void clearHistoryData(){
    	historyData.clear();
    }
	public void reqHistoricalDataStrategy(Contract contract, String endDateTime, int duration, String durUnit,
			String barSize, String wTS, boolean rthOnly, StrategyHisDataRow row) {
		if (!checkConnection())
			return;

		int reqId = m_reqId++;
		m_hisDataMapStrategy.put(reqId, row);
		String duraString = duration + " " + durUnit.charAt(0);
		m_client.reqHistoricalData(reqId, contract, endDateTime, duraString, barSize, wTS, rthOnly ? 1 : 0, 2,
...
//					longDate = new GregorianCalendar(year - 1900, month - 1, day).getTimeInMillis() / 1000;
	...
	public void hisDataStrategy(int reqId, String date, double open, double high, double low, double close, int volume,
			int count, double wap, boolean hasGaps) {
		StrategyHisDataRow row = m_hisDataMapStrategy.get(reqId);
		if (row != null) {
			if (date.startsWith("finished")) {
				row.historicalDataEnd();
				// handler.historicalDataEnd();
			} else {
				long longDate;
				if (date.length() == 8) {
