...
public class MarketValueAndPortfolioPanel extends JPanel implements INewTab, IAccountHandler {
	...
	MarketValueAndPortfolioPanel() {
        ...
        public double getCashBalanceUSD(){
           for (MktValRow i : m_mktValModel.m_list) {
		if (i.m_currency.equals("USD") ){
                    for (MarketValueTag j : i.m_map.keySet()) {
                        if (j.toString().equals("CashBalance")){
                             return Double.parseDouble(i.m_map.get(j));
                        }                                                   
                    }                 
                }
           }
